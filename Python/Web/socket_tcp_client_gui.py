...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        self.textbox_recv_msg = scrolledtext.ScrolledText(self, width=30, height=5)
        self.textbox_recv_msg.grid(row=0, column=0, padx=5, pady=5)

        ttk.Label(self, text='IP:').grid(row=0, column=1, sticky='NW', padx=5, pady=5)
        ...
        ttk.Label(self, text='Port:').grid(row=0, column=1, sticky='SW', padx=5, pady=5)
        ...
        button_send = ttk.Button(self, text='Send', width=10, command=self.send)
    ...
    def on_connect(self):
        ...
        self.textbox_recv_msg.insert(tk.END, (f'成功连接到' f'服务器({self.server_ip_info})\n'))
